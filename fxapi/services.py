...
def get_expiring_contracts(exp_date):
    expiring_contracts = models.Option.objects.filter(expiration=exp_date).order_by('symbol')
...
def write_cab_to_db(option, cab_data):
    ...
    curr_contracts = models.Option.objects.filter(symbol=option.symbol).filter(option_type=option.option_type).filter(
        expiration__gte=option.expiration).order_by('expiration')
    cab_instance, created = models.ComfortZones.objects.get_or_create(symbol=option.symbol,
